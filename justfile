...
# Task runner for the standalone Mistral realtime WASM backend. Mirrors the
# recipe names used by the main super-stt repo (`just check`, `just ci`, etc.).

# Default: build the component
default: build-component

# Build the wit-bindgen component (wasm32-wasip2). Requires the target:
...
# only sees the pure helpers (the wit-bindgen handler is cfg'd out), so the
# component's own code is linted here. Gating: `-D warnings`.
check *args:
